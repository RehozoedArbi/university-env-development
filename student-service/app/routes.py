...
def health():
    try:
        ...
        current_app.logger.error({"service": "student-service", "event": "health_check_failed"})
        return jsonify({"status": "unhealthy", "service": "student-service"}), 503
...
def create_student():
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error({"service": "student-service", "event": "db_error", "action": "create_student"})
...
def update_student(student_id: int):
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error({"service": "student-service", "event": "db_error", "action": "update_student"})
        return jsonify({"error": "Erreur interne, réessayez plus tard"}), 500
...
def student_exists(student_id: int):
    exists = db.session.query(Student.id).filter_by(id=student_id).first() is not None
...
def available_courses(student_id: int):
    ...
    try:
        response = call_service(
            _logger(), "student-service", "teacher-admin-service", "GET", url,
...
def student_enrollments(student_id: int):
    ...
    url = f"{current_app.config['ENROLLMENT_SERVICE_URL']}/enrollments?student_id={student_id}"
    try:
        response = call_service(
            _logger(), "student-service", "enrollment-service", "GET", url,
            trace_id=_trace_id(), timeout=current_app.config["HTTP_TIMEOUT_SECONDS"],
        )
    except DownstreamError:
        return jsonify({"error": "Le service des inscriptions est momentanément indisponible"}), 502
...
def student_grades(student_id: int):
    ...
    url = f"{current_app.config['ENROLLMENT_SERVICE_URL']}/grades?student_id={student_id}"
    try:
        response = call_service(
            _logger(), "student-service", "enrollment-service", "GET", url,
            trace_id=_trace_id(), timeout=current_app.config["HTTP_TIMEOUT_SECONDS"],
